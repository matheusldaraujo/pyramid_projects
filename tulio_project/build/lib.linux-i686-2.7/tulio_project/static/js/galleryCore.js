...
$(function() {
	...
	function initEvents(){
		...
		$pg_photos_elem.bind('mouseenter',function(e){
			...
			deleteImgButton = $('<input type="button" class="submit button removeImg"  id="removeImg" style="border-radius: 5px;top:90%;padding:5px;width: 100%;margin: 0px;" value="'+ remove +'" name="delete.submitted"/>')

			if($(logged).val() == "1"){
				deleteImgButton.hide().insertAfter($img).fadeIn();
			}
			
			deleteImgButton.bind('click',function(){
				actual = deleteImgButton
				img = actual.siblings("img")
				path = actual.siblings("img").attr("src")
				$.get("/removeImg",{path:path},function(data){
				if (data.message == "ok"){
						actual.parent().parent().fadeOut()
			...
			addComment = $('<span class="addComment" style="border-radius: 5px;padding: 10px;">'+ descr +'</span>')

			addComment.hide().insertAfter($img).fadeIn();
